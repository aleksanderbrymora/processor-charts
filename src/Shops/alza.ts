...
export class Alza {
	...
	async start(page: pptr.Page): Promise<ShopData> {
		...
		await this.page.goto(
			'https://www.alza.cz/nejprodavanejsi-nejlepsi-procesory/18842843.htm',
		);
		await this.page.waitForSelector('a.name');
		const data: ProcessorData[] = await this.page.evaluate(() => {
			const processors = document.querySelectorAll<HTMLDivElement>('a.name');
